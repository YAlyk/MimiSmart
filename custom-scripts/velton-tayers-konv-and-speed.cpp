/*
{
      ...
      vars:[
            ...
            {name:"SPEED03", type:"devices-list",filter:["lamp"], required:true,  desc:"Скорость 3"},
...
V-ID/s:5{
      if(([COND.0]%2)==1){
            ...
            // Скорости
            if(([VALVE.0]&1)==1){
                  if([COND.4]==0){setStatus(SPEED01, 0); setStatus(SPEED02, 0); setStatus(SPEED03, 0);}
                  else if([COND.4]==1){setStatus(SPEED03, 0); setStatus(SPEED02, 0); setStatus(SPEED01, 1);}
                  else if([COND.4]==2){setStatus(SPEED01, 0); setStatus(SPEED03, 0); setStatus(SPEED02, 1);}
                  else if([COND.4]==3){setStatus(SPEED01, 0); setStatus(SPEED02, 0); setStatus(SPEED03, 1);}
            } else {setStatus(SPEED01, 0); setStatus(SPEED02, 0); setStatus(SPEED03, 0);}
      }
      else {
            if([VALVE.5]==!254) {
                  setStatus(SPEED01, 0);
