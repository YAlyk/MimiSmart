...
{name:"DELTA0", type:"number", defaultValue:1, required:true, desc:"Дельта для 1 скорости", min:0},
{name:"DELTA1", type:"number", defaultValue:1, required:true, desc:"Дельта для 2 скорости", min:0},
{name:"DELTA2", type:"number", defaultValue:1, required:true, desc:"Дельта для 3 скорости", min:0},
{name:"V00",type:"devices-list",required:true, filter:["valve-heating"],descWidth:150, desc:"Батарея"},
{name:"D00",type:"devices-list",required:true, filter:["lamp"],desc:"Скорость 1"},
{name:"D01",type:"devices-list",required:true, filter:["lamp"],desc:"Скорость 2"},
{name:"D02",type:"devices-list",required:true, filter:["lamp"],desc:"Скорость 3"},
...
V-ID/s:5
{
  ...
  if( ([V00.2]-DELTA0) >= [T00.1] ) {setStatus(D00, 1);} // Если заданная температура выше на DELTA чем температура на датчике - включаем вентилятор
